...
def select_product_by_text(company):
    ...
    if selected_product_name:
        ...
        #st.write(f"คุณเลือกสินค้า **{selected_product_name}**")
        st.markdown(f'<span style="font-weight: bold;">คุณเลือกสินค้า: <span style="background-color: orange; padding: 2px 5px; border-radius: 5px; color: black;">{selected_product_name}</span></span>', unsafe_allow_html=True)
        st.markdown("---")
