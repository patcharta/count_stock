...
def select_product_by_qr(company):
    ...
    qr_image_file = st.file_uploader("อัปโหลดภาพ QR Code ของสินค้า")

    if qr_image_file is not None:
        qr_code = decode_qr_code(qr_image_file)
        if qr_code:
...
def decode_qr_code(qr_image_file):
    try:
        ...
        if qr_code:
            ...
            st.warning("ไม่พบข้อมูล QR Code ในภาพที่อัปโหลด")
            return None
    except Exception as e:
        st.error(f"เกิดข้อผิดพลาดในการอ่าน QR Code: {e}")
        return None
